...
export default function Home() {
  ...
  return (
    <div className="flex flex-col w-full animate-in fade-in duration-500">
      ...
      {/* Hero Section */}
      <section className="relative w-full pt-6 pb-16 px-3 md:pt-12 md:pb-24 md:px-8 bg-gradient-to-br from-background via-background to-primary/20 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto text-center mt-4 md:mt-12">
          <div className="inline-flex items-center gap-1.5 px-3 py-1.5 md:px-4 md:py-2 rounded-full bg-muted text-muted-foreground text-xs md:text-sm font-medium mb-3 md:mb-6 backdrop-blur-md border border-border">
            <Shield className="w-3.5 h-3.5 md:w-4 md:h-4 text-secondary" />
            <span>Privacy-first community accountability</span>
          </div>
          
          <h1 className="text-[1.75rem] leading-tight md:text-6xl lg:text-7xl font-display font-extrabold text-foreground tracking-tight mb-3 md:mb-6">
            Bridging trust through <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-teal-400">transparency.</span>
          </h1>
          
          <p className="text-sm md:text-xl text-muted-foreground max-w-2xl mx-auto mb-6 md:mb-10 leading-relaxed">
      ...
      <section className="relative z-20 max-w-5xl mx-auto px-3 md:px-8 -mt-10 md:-mt-16 w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-0 md:gap-6">
          ...
          <Link href="/report" className="block group">
            <div className="bg-card p-4 md:p-8 md:rounded-3xl shadow-xl dark:shadow-none border border-border md:hover:border-primary/30 transition-all duration-300 h-full flex items-center md:flex-col gap-4 md:gap-0 border-b border-border md:border-b-0 md:group-hover:-translate-y-1 first:rounded-t-2xl md:first:rounded-t-3xl last:rounded-b-2xl md:last:rounded-b-3xl shadow-sm md:shadow-xl">
              ...
              <div className="flex-1 md:flex md:flex-col md:flex-1">
                <h2 className="text-lg md:text-2xl font-bold text-foreground mb-0.5 md:mb-2">Report an Incident</h2>
                <p className="text-muted-foreground text-sm md:text-base md:mb-8 md:flex-1 line-clamp-2 md:line-clamp-none">
              ...
              <ChevronRight className="w-5 h-5 text-muted-foreground md:hidden shrink-0" />
            </div>
          </Link>
          ...
          <Link href="/recognize" className="block group">
            <div className="bg-card p-4 md:p-8 md:rounded-3xl shadow-xl dark:shadow-none border border-border md:hover:border-secondary/30 transition-all duration-300 h-full flex items-center md:flex-col gap-4 md:gap-0 border-b border-border md:border-b-0 md:group-hover:-translate-y-1 first:rounded-t-2xl md:first:rounded-t-3xl last:rounded-b-2xl md:last:rounded-b-3xl shadow-sm md:shadow-xl">
              ...
              <div className="flex-1 md:flex md:flex-col md:flex-1">
                <h2 className="text-lg md:text-2xl font-bold text-foreground mb-0.5 md:mb-2">Recognize an Officer</h2>
                <p className="text-muted-foreground text-sm md:text-base md:mb-8 md:flex-1 line-clamp-2 md:line-clamp-none">
      ...
      <section className="max-w-6xl mx-auto px-3 md:px-8 py-8 md:py-16 w-full">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-5 md:mb-8 gap-3 md:gap-4">
          <div>
            <h2 className="text-xl md:text-3xl font-display font-bold text-foreground flex items-center gap-2">
              <Activity className="w-5 h-5 md:w-7 md:h-7 text-primary" />
              Community Pulse
            </h2>
            <p className="text-muted-foreground text-sm md:text-base mt-1 md:mt-2">Aggregate, anonymized data for the current month.</p>
          </div>
          <Link href="/map">
            <div className="inline-flex items-center justify-center px-4 py-2 md:px-5 md:py-2.5 rounded-full bg-muted text-muted-foreground text-sm md:text-base font-medium hover:bg-accent hover:text-accent-foreground transition-colors cursor-pointer">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
          {isLoading ? (
             Array(4).fill(0).map((_, i) => (
               <div key={i} className="bg-card p-4 md:p-6 rounded-xl md:rounded-2xl border border-border h-24 md:h-32 animate-pulse flex flex-col justify-center">
                 <div className="w-1/2 h-3 md:h-4 bg-muted rounded mb-3 md:mb-4"></div>
                 <div className="w-3/4 h-6 md:h-8 bg-muted rounded"></div>
               </div>
             ))
          ) : (
            <>
...
function StatCard({ title, value, icon: Icon, trend, color }: { title: string, value: string | number, icon: any, trend: string, color: 'blue' | 'teal' | 'orange' | 'indigo' }) {
  ...
  return (
    <div className="bg-card p-3 md:p-6 rounded-xl md:rounded-2xl border border-border hover:border-border/80 transition-shadow dark:shadow-none">
      ...
      <div>
        <h3 className="text-xs md:text-sm font-medium text-muted-foreground mb-0.5 md:mb-1">{title}</h3>
        <p className="text-xl md:text-3xl font-bold text-foreground">{value}</p>
        <p className="text-[10px] md:text-xs font-medium text-muted-foreground mt-1 md:mt-2">{trend}</p>
      </div>
    </div>
  );
}
