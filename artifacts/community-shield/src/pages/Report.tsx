...
export default function Report() {
  ...
  const handleSubmit = () => {
    // Defaulting missing required fields purely for frontend robustness
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 md:py-12 w-full animate-in fade-in">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-3xl md:text-4xl font-display font-bold text-slate-900 mb-2">Report an Incident</h1>
        <p className="text-slate-600">Your privacy is protected. Identifying information is not publicly displayed.</p>
      </div>
      ...
      <div className="bg-white rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6 md:p-10 relative overflow-hidden">
        <AnimatePresence mode="wait">
          {step === 1 && (
            <motion.div key="step1" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
              <h2 className="text-2xl font-bold text-slate-900 mb-6">What type of incident?</h2>
                ...
                {[
                  ...
                  return (
                    ...
                    >
                      ...
                      <span className={`font-semibold text-lg ${isSelected ? 'text-primary' : 'text-slate-700'}`}>
                        {type.label}
                      </span>
                    </button>
                  );
          ...
          {step === 2 && (
            <motion.div key="step2" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
              <h2 className="text-2xl font-bold text-slate-900">Officer & Location Details</h2>
              <p className="text-slate-500 mb-6">This helps us verify the incident. All fields are optional but highly encouraged.</p>
              
              <div className="space-y-5">
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Badge Number</label>
                  <input 
                    type="text" 
                ...
                <div>
                  <label className="block text-sm font-semibold text-slate-700 mb-2">Precinct / Division</label>
                  <input 
                    type="text" 
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Police Vehicle Plate</label>
                    <input 
                      type="text" 
                  ...
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">Citation Number</label>
                    <input 
                      type="text" 
          ...
          {step === 3 && (
            <motion.div key="step3" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
              <h2 className="text-2xl font-bold text-slate-900">Interaction Survey</h2>
              <p className="text-slate-500 mb-6">Simple yes/no questions to establish behavioral patterns.</p>
          ...
          {step === 4 && (
            <motion.div key="step4" initial={{ x: 20, opacity: 0 }} animate={{ x: 0, opacity: 1 }} exit={{ x: -20, opacity: 0 }} className="space-y-6">
              <h2 className="text-2xl font-bold text-slate-900">Description & Submit</h2>
              <p className="text-slate-500 mb-6">Please provide any additional details about the incident.</p>
              
              <div>
                <label className="block text-sm font-semibold text-slate-700 mb-2">Incident Description <span className="text-red-500">*</span></label>
                <textarea 
          ...
          {step === 5 && (
            <motion.div key="step5" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="py-12 text-center flex flex-col items-center">
              <div className="w-24 h-24 bg-green-100 text-green-600 rounded-full flex items-center justify-center mb-6">
                <CheckCircle2 className="w-12 h-12" />
              </div>
              <h2 className="text-3xl font-display font-bold text-slate-900 mb-4">Report Submitted</h2>
        ...
        {step < 5 && (
          <div className="mt-10 pt-6 border-t border-slate-100 flex justify-between items-center">
            {step > 1 ? (
              <button 
                onClick={handlePrev}
                className="px-6 py-2.5 rounded-full font-medium text-slate-600 hover:bg-slate-100 transition-colors flex items-center gap-2"
              >
            ...
            {step < 4 ? (
              <button 
                ...
                className="px-8 py-2.5 bg-primary text-white rounded-full font-semibold shadow-md shadow-primary/25 hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
              >
              ...
              <button 
                ...
                className="px-8 py-2.5 bg-slate-900 text-white rounded-full font-semibold shadow-md shadow-slate-900/25 hover:shadow-lg hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center gap-2"
              >
...
function BinaryQuestion({ label, value, onChange, dangerYes = false }: { label: string, value?: boolean | null, onChange: (val: boolean) => void, dangerYes?: boolean }) {
  return (
    <div className="bg-slate-50 border border-slate-100 rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4">
      <p className="font-medium text-slate-800 flex-1">{label}</p>
      <div className="flex items-center gap-2 shrink-0">
        <button 
          onClick={() => onChange(true)}
          className={`flex-1 md:w-24 px-4 py-2 rounded-xl font-medium border-2 transition-colors flex items-center justify-center gap-1 ${
            value === true 
